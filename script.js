...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (saved.schoolName) {
        ...
        schoolSelect.dataset.saved = saved.schoolName; // 後で復元に使う
    }

    // index.html にしか存在しない要素を確認
    const schoolType = document.getElementById("schoolType");
    const schoolSelect = document.getElementById("schoolSelect");
    const btn = document.getElementById("goQuestions");

    if (!schoolType || !schoolSelect || !btn) return; // 他ページは無視
...
if (document.getElementById("questionContainer")) {
     ...
     * 判定
     **********************/
    function evaluate(data, schoolScore) {
        ...
        data.majorQuestions.forEach(q => {
            q.subquestions.forEach(sub => {
                ...
                if (val && val.value === "wrong" && allowed.includes(sub.difficulty)) {
                    if (!results[sub.unit]) results[sub.unit] = [];
                    results[sub.unit].push(`${q.id}-${sub.no}`);
                }
            });
        });
